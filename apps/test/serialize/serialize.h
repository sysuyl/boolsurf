...
inline Serializer make_serializer(
    const std::string& filename, bool save, size_t buffer_capacity) {
  Serializer srl;
  srl.is_writer = save;
...
inline Serializer make_reader(
    const std::string& filename, size_t buffer_capacity) {
  return make_serializer(filename, false, buffer_capacity);
}

inline Serializer make_writer(
    const std::string& filename, size_t buffer_capacity) {
  return make_serializer(filename, true, buffer_capacity);
}

// Release resources.
inline void close_serializer(Serializer& srl) {
  if (srl.buffer) {
...
inline size_t serialize_size(Serializer& srl, Container& vec) {
  size_t size;
  if (srl.is_writer) {
    size = vec.size();
    serialize(srl, size);
...
template <typename Type>
inline void serialize_vector(Serializer& srl, std::vector<Type>& vec) {
...
inline void serialize_vector_generic(Serializer& srl, Vector& vec) {
  size_t count = vec.size();
  serialize(srl, count);
  if (count == 0) return;
  if (srl.is_writer) {
...
inline void serialize_vector_custom(
    Serializer& srl, std::vector<Type>& vec, Serialize_func serialize_obj) {
  size_t count = vec.size();
  serialize(srl, count);
  if (count == 0) return;
  if (srl.is_writer) {
...
// Serialize std::vector of structs with custom serialize function
template <typename Type>
inline void serialize_vector_custom(Serializer& srl, std::vector<Type>& vec) {
  size_t count = vec.size();
  serialize(srl, count);
  if (count == 0) return;
  if (srl.is_writer) {
...
inline void serialize_string(Serializer& srl, std::string& str) {
...
inline void load_from_file(
    const std::string& filename, Type& object, int buffer_size = 1048576) {
...
template <typename Type>
inline Type make_from_file(
    const std::string& filename, int buffer_size = 1048576) {
  Type object;
